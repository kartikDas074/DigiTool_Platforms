...
const Product = ({count,setCount,card,setCard}) => {
    ...
    return (
        <div className='w-full max-w-[95vw] md:max-w-[80vw] mx-auto py-[60px] flex flex-col justify-center items-center gap-4  '>
            ...
            <div className='bg-gray-50 px-2.5 py-2 space-x-1 shadow-2xl rounded-[20px] mb-15 md:mb-20'>
               ...
               <button className={`btn  ${!product?'bg-linear-to-r from-[#4F39F6] to-[#9514FA]  text-[#FFFFFF] font-semibold ':'bg-linear-to-r from-[#4F39F6] to-[#9514FA] bg-clip-text text-[18px] font-medium text-transparent'} rounded-[25px]`}  onClick={()=>{setProduct(false)}}>Cart (<span>{count}</span>)</button>
            </div>
            <div  className=' w-full'>
            {
            product?<ProductList total={total} setTotal={setTotal} count={count} setCount={setCount} card={card} setCard={setCard}></ProductList>:<CartInfo product={setProduct} total={total} setTotal={setTotal} count={count} setCount={setCount} card={card} setCard={setCard}></CartInfo>
            }
            </div>
